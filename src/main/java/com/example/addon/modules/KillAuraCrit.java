...
public class KillAuraCrit extends Module {
    ...
    private final Setting<Double> Critvel = sgDelay.add(new DoubleSetting.Builder()
        .name("Crit-Vel")
        .description("How many ticks to wait before hitting an entity after switching hotbar slots.")
        .defaultValue(-0.5)
        .min(-10)
    ...
    public KillAuraCrit() {
        super(Categories.Combat, "kill-aura-crit", "Attacks specified entities around you.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (mc.player.isOnGround() && !mc.player.isSneaking()) mc.player.jump();
    ...
    private void attack(Entity target) {
        ...
        if(mc.player.getVelocity().y > Critvel.get()) return;
